...
![Set-LLM mask transition diagram](attention.png)
...
## How We Turn Pairwise Judging into a Set Input

- Each example has:
  - one **shared user prompt**
  - two **candidate responses**
- We serialize them as:
  - shared prompt prefix
  - candidate response 1 as one set element
  - candidate response 2 as another set element
- The two responses should be treated as an **unordered set**
- Desired behavior:
  - swapping the two responses should only swap the prediction label
  - the internal score geometry should stay symmetric

---

## What `setllm` Changes

- `setllm` keeps the original Set-LLM idea:
  - add **SetPE** so sibling elements do not depend on presentation order
  - use **SetMask** to block direct attention across different candidates
- Inside one candidate response:
  - prompt-side tokens can still attend bidirectionally within the element
- Across two different candidates:
  - attention is blocked, so one answer cannot directly leak into the other

Intuition:

- candidate A and candidate B both read the same shared prompt
- but neither candidate can "peek" at the other's response tokens
- this removes many order shortcuts that exist in vanilla decoder prompts

---

## What `setcausal` Changes

- `setcausal` keeps the same high-level set decomposition:
  - shared prompt
  - isolated candidate elements
- But **inside each candidate**, it restores a decoder-style **causal** mask
- So `setcausal` is:
  - set-aware **across** responses
  - causal **within** each response

Why this matters:

- modern judge backbones are decoder-only LLMs
- they were pretrained with causal next-token prediction
- long multi-turn responses may therefore be represented more naturally by
  a causal local mask than by full prompt-side bidirectional visibility

---

## `setllm` vs `setcausal`

| Property | `setllm` | `setcausal` |
| --- | --- | --- |
| Candidate isolation | yes | yes |
| Uses SetPE | yes | yes |
| Cross-response leakage blocked | yes | yes |
| Within-response mask | prompt-side SetMask | decoder causal |
| Closest to original Set-LLM paper | yes | partial |
| Closest to decoder pretraining | weaker | stronger |

Working interpretation:

- `setllm` is the most faithful adaptation of the paper design
- `setcausal` is the judge-specific variant we introduced for long responses
- the empirical question is not only "which is more invariant"
- it is also "which inductive bias better matches long-form judging"
...
- **Task:** pairwise judging
- **Datasets prepared or targeted:** JudgeBench, MT-Bench
- **Training:** LoRA on decoder-only HF models, currently Gemma 2B, Gemma 7B, Qwen 3.5
